THIS IS THE 2.0 BRANCH AND IS NO LONGER DEVELOPED

This branch is frozen, it will probably (maybe?) get bugfixes but won't change significantly. It is here for anyone that needs to maintain compatibility with an existing codebase, if you are new to Mingo than you should use the `master` branch as that is the most up-to-date version of the code.
