 ...
 *
 *  This interface has a few quirks, for example:
 *    - sorting does not work, Lucene does its own sorting, so why would you sort?
 *    - unique fields supported
 *    - certain criteria searches aren't supported
...
class MingoLuceneInterface extends MingoInterface {
  ...
  protected function _get($table,$where_criteria){
    ...
    foreach($list as $i => $hit){
      ...
      ///\out::e($table['lucene']->isDeleted($hit->id));
  ...
  protected function normalizeCriteria(MingoTable $table,MingoCriteria $where_criteria = null){
    ...
    if($where_criteria !== null){
      ...
      foreach($criteria_where as $name => $map){
        ...
        if($subquery){ $query->addSubquery($subquery,$required); }//method
  ...
  protected function removePath($path){
    ...
    $ret_bool = true;
    $path_iterator = new RecursiveIteratorIterator(
      new RecursiveDirectoryIterator(
        $path,
        RecursiveIteratorIterator::CHILD_FIRST | FilesystemIterator::CURRENT_AS_FILEINFO | FilesystemIterator::SKIP_DOTS
      )
    );
    foreach($path_iterator as $file){
      ...
      if($file->isDir()){
        ...
        $ret_bool = $this->removePath($file_path);
